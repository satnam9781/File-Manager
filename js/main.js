...
window.ondblclick = function(e) {
  ...
  // Open directory when double clicked on span-elements or icons in list item
    ...
    if (e.target.parentElement.getAttribute("data-type") == "file") {
      editFile();
...
// To give hover effect to dropdown caret
$$(".dropdown-item").forEach(el => {
  if (getChildNum(el) == 0) {
    el.addEventListener("mouseenter", e => {
      e.target.parentElement.className = "dropdown-item-container hovered";
    });
    el.addEventListener("mouseleave", e => {
      e.target.parentElement.className = "dropdown-item-container";
    });
  }
});
...
function createTree() {
  ajax("createTree=true&path=null", function(obj) {
    ...
    response = response.replace(
      /&0;/g,
      "<img class='svg-icon' src='icons/folder.svg'>"
    );
    response = response.replace(
      /&1;/g,
      "<li class='tree-item'><img onclick='expandTree(this)' src='icons/caret-right.svg' class='icon'>"
    );
    response = response.replace(
      /&2;/g,
      "<li class='tree-item opened'><img onclick='expandTree(this)' src='icons/caret-down.svg' class='icon'>"
    );
    response = response.replace(
      /&3;/g,
...
function expandTree(el) {
  if (el.parentElement.nextElementSibling) {
    ...
    if (
      ...
      el.src = "icons/caret-right.svg";
    } else if (tree.tagName == "UL" && tree.style.display == "none") {
      tree.style.display = "block";
      el.src = "icons/caret-down.svg";
...
function listDirectories(path, position) {
  ajax("listDirectories=true&path=" + encodeURIComponent(path), function(obj) {
    if (obj.response == "") {
      ...
      position.firstChild.src = "icons/caret-down.svg";
...
function toggleMoreOptions() {
  $("#moreOptions").style.display = countSelected() > 0 ? "inline" : "none";
  $("#renameButton").style.display = countSelected() == 1 ? "block" : "none";
...
function changeView(view) {
  ...
  // Details View
    ...
    $(".view-icon").src = "./icons/details.svg";
    $(".main-container").style.height = window.innerHeight - 104 + "px";
