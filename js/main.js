...
let clickEvent = window.innerWidth <= 576 ? "click" : "dblclick";

// To scan the current directory by default on page load
scanPath("./");

window.onresize = function() {
  let previousEvent = window.innerWidth <= 576 ? "dblclick" : "click";
  clickEvent = window.innerWidth <= 576 ? "click" : "dblclick";
  document.body.removeEventListener(previousEvent, openDirectory);
  document.body.addEventListener(clickEvent, openDirectory);
};
...
window.onmousedown = function(e) {
  ...
  if (
   ...
   if (e.target == $(".main-container") || e.target == $(".list-group")) {
...
document.body.onclick = function(e) {
  ...
  if (
    e.target.className == "more-options" ||
    e.target.className == "material-icons more-icon"
  )
    return false;
...
function openDirectory(e) {
  ...
  if (
    ...
    if (e.target.getAttribute("data-type") == "insert_drive_file") {
      editFile();
...
// To show the options for an item in mobile devices
function showMoreOptions(item) {
